...
const GRAND_TOTAL_STYLES = {
  '#28a745': { bg: '#e8f5e9', color: '#2e7d32' },
  '#ffc107': { bg: '#fff3e0', color: '#e65100' },
};
const getGrandTotalStyle = (report) => GRAND_TOTAL_STYLES[getGrandTotalColor(report)] || { bg: '#ffebee', color: '#c62828' };
...
const RemarksCell = ({ text }) => (
  text ? (
    <div style={{ background: '#fff9e6', border: '1px solid #ffc107', borderRadius: '8px', padding: '10px', maxHeight: '80px', overflow: 'auto', lineHeight: '1.4', color: '#333', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
      {text}
    </div>
  ) : (
    <span style={{ color: '#999', fontStyle: 'italic' }}>No remarks</span>
  )
);

// One row of the pending-reports table — extracted from the previous
// inline .map() callback to keep the parent component's complexity down.
const PendingReportRow = ({ report, index, onViewRemarks }) => {
  const foMax = report.placementApplicable === 'no' ? 35 : 30;
  const dpMax = report.placementApplicable === 'no' ? 45 : 40;
  const ppMax = 15;
  const mpMax = report.placementApplicable === 'no' ? 20 : 15;
  ...
  const placementColor = report.placementApplicable === 'no' ? '#999' : placementInfo.color;
  const placementValue = report.placementApplicable === 'no' ? 'N/A' : parseFloat(report.placementScore || 0).toFixed(2);
  ...
  return (
    <tr style={{
      ...
      <td style={{ textAlign: 'center' }}><CenterTypeBadge type={report.centerType} /></td>
      ...
      <td style={{ textAlign: 'center', fontWeight: 'bold', fontSize: '15px', color: placementColor }}>
        {placementValue}
      </td>
      ...
      <td style={{ textAlign: 'center', fontWeight: 'bold', fontSize: '18px', color: getGrandTotalColor(report) }}>
        {report.grandTotal}
      </td>
      <td style={{ textAlign: 'center', fontWeight: 'bold', color: getGrandTotalColor(report), fontSize: '13px' }}>
        {getAuditStatus(report)}
      </td>
      <td style={{ textAlign: 'left', fontSize: '12px', padding: '10px', maxWidth: '200px' }}>
        <RemarksCell text={report.remarksText} />
      </td>
      <td style={{ textAlign: 'center', fontSize: '13px' }}>{report.submittedDate || '-'}</td>
      <td style={{ textAlign: 'center', padding: '10px' }}>
        <button
          onClick={() => onViewRemarks(report)}
          style={{
...
// ── Remarks detail modal sub-pieces ──────────────────────────────
const ModalCenterInfoGrid = ({ r }) => {
  const { bg: statusBg, color: statusColor } = getGrandTotalStyle(r);
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px', fontSize: '13px' }}>
      <div><strong>Center Code:</strong> <span style={{color: '#667eea', fontWeight: 'bold'}}>{r.centerCode}</span></div>
      <div><strong>Center Name:</strong> {r.centerName}</div>
      <div><strong>Project Name:</strong> {r.projectName || '-'}</div>
      <div><strong>ZM Name:</strong> {r.zmName || '-'}</div>
      <div><strong>Region Head:</strong> {r.regionHeadName || '-'}</div>
      <div><strong>Area Manager:</strong> {r.areaManager || '-'}</div>
      <div><strong>Cluster Manager:</strong> {r.clusterManager || '-'}</div>
      {r.placementApplicable === 'yes' && r.placementCoordinator && (
        <div><strong>Placement Coordinator:</strong> {r.placementCoordinator}</div>
      )}
      {r.placementApplicable === 'yes' && r.seniorManagerPlacement && (
        <div><strong>Sr. Manager Placement:</strong> {r.seniorManagerPlacement}</div>
      )}
      {r.placementApplicable === 'yes' && r.nationalHeadPlacement && (
        <div><strong>National Head Placement:</strong> {r.nationalHeadPlacement}</div>
      )}
      <div><strong>Center Head:</strong> {r.centerHeadName || '-'}</div>
      <div><strong>Center Type:</strong> <CenterTypeBadge type={r.centerType} small /></div>
      <div><strong>Location:</strong> {r.location || r.geolocation || '-'}</div>
      <div><strong>Audited By:</strong> {r.auditedBy || '-'}</div>
      <div><strong>Audit Period:</strong> {r.auditPeriod || '-'}</div>
      <div><strong>Financial Year:</strong> <span style={{color: '#667eea', fontWeight: 'bold'}}>{r.financialYear || 'FY26'}</span></div>
      <div><strong>Audit Date:</strong> {r.auditDate || '-'}</div>
      <div><strong>Grand Total:</strong> <span style={{ fontSize: '15px', fontWeight: 'bold', color: getGrandTotalColor(r) }}>{parseFloat(r.grandTotal || 0).toFixed(2)}/100</span></div>
      <div><strong>Status:</strong> <span style={{ padding: '2px 8px', borderRadius: '8px', fontSize: '11px', fontWeight: 'bold', background: statusBg, color: statusColor }}>{getAuditStatus(r)}</span></div>
    </div>
  );
};

const ModalRemarksSection = ({ r }) => (
  <>
    {r.overallRemarks && (
      <div style={{ background: '#fff9e6', padding: '15px 20px', borderRadius: '8px', marginBottom: '20px', border: '2px solid #ffc107' }}>
        <h4 style={{ margin: '0 0 10px 0', color: '#e65100', fontSize: '16px' }}>⭐ Overall Remarks (From User):</h4>
        <p style={{ margin: 0, fontSize: '14px', color: '#333', lineHeight: '1.6' }}>{r.overallRemarks}</p>
      </div>
    )}
    {r.centerRemarksBy && (
      <div style={{ background: '#e3f2fd', padding: '12px 16px', borderRadius: '8px', marginBottom: '20px', fontSize: '14px', color: '#1565c0' }}>
        <strong>📝 Remarks by:</strong> {r.centerRemarksBy} |
        <strong> Date:</strong> {r.centerRemarksDate || '-'}
      </div>
    )}
  </>
);

const ModalCheckpointRow = ({ cp, idx, cpData }) => (
  <tr style={{ borderBottom: '1px solid #e0e0e0', background: idx % 2 === 0 ? 'white' : '#f8f9fa' }}>
...
const ModalAreaSection = ({ area, number, selectedRemarks }) => {
  let areaTotal = 0;
  ...
  return (
    <React.Fragment>
      ...
      {area.checkpoints.map((cp, idx) => (
        <ModalCheckpointRow key={cp.id} cp={cp} idx={idx} cpData={selectedRemarks.data?.[cp.id] || {}} />
      ))}
      {!area.isNA && (
...
const ModalCheckpointTable = ({ selectedRemarks }) => {
  const areas = getCheckpointsByArea(selectedRemarks.centerType || 'CDC', selectedRemarks.placementApplicable || 'yes');
  return (
    <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '1200px' }}>
      ...
      <tbody>
        {MODAL_AREAS_ORDER.map(({ key, number }) => (
          <ModalAreaSection key={key} area={areas[key]} number={number} selectedRemarks={selectedRemarks} />
        ))}
      </tbody>
    </table>
  );
...
// Full remarks/approve/reject modal — extracted from the parent component's
// return statement to keep its own complexity down.
const RemarksDetailModal = ({ r, loading, onClose, onApprove, onReject }) => (
  <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 10000 }}>
    <div style={{ background: 'white', borderRadius: '15px', width: '95%', maxWidth: '1400px', maxHeight: '90vh', overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
      <div style={{ background: 'linear-gradient(135deg, #11998e 0%, #38ef7d 100%)', padding: '20px 25px', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <h3 style={{ margin: 0, fontSize: '20px' }}>📊 Complete Audit Report - Detailed View</h3>
          <p style={{ margin: '5px 0 0', fontSize: '14px', opacity: 0.9 }}>{r.centerName} ({r.centerCode})</p>
        </div>
        <button onClick={onClose} style={{ background: 'rgba(255,255,255,0.2)', border: 'none', color: 'white', fontSize: '24px', cursor: 'pointer', borderRadius: '50%', width: '40px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
      </div>

      <div style={{ padding: '20px', maxHeight: '70vh', overflowY: 'auto', overflowX: 'auto' }}>
        <div style={{ background: 'linear-gradient(135deg, #e3f2fd 0%, #bbdefb 100%)', padding: '20px', borderRadius: '12px', marginBottom: '20px', border: '2px solid #2196f3' }}>
          <h4 style={{ margin: '0 0 15px 0', color: '#1976d2', fontSize: '16px', fontWeight: 'bold' }}>🏢 Center Information</h4>
          <ModalCenterInfoGrid r={r} />
        </div>

        <ModalRemarksSection r={r} />

        <ModalCheckpointTable selectedRemarks={r} />
      </div>

      <div style={{ padding: '15px 25px', borderTop: '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <button onClick={onClose} style={{ background: '#999', color: 'white', border: 'none', padding: '12px 30px', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: 'bold' }}>
          ← Back
        </button>
        <div style={{ display: 'flex', gap: '15px' }}>
          <button
            onClick={onReject}
            disabled={loading}
            style={{ background: 'linear-gradient(135deg, #f44336 0%, #d32f2f 100%)', color: 'white', border: 'none', padding: '12px 35px', borderRadius: '8px', cursor: loading ? 'not-allowed' : 'pointer', fontSize: '15px', fontWeight: 'bold', boxShadow: '0 4px 15px rgba(244, 67, 54, 0.3)', opacity: loading ? 0.6 : 1 }}
          >
            ✕ Reject Report
          </button>
          <button
            onClick={onApprove}
            disabled={loading}
            style={{ background: 'linear-gradient(135deg, #4caf50 0%, #388e3c 100%)', color: 'white', border: 'none', padding: '12px 35px', borderRadius: '8px', cursor: loading ? 'not-allowed' : 'pointer', fontSize: '15px', fontWeight: 'bold', boxShadow: '0 4px 15px rgba(76, 175, 80, 0.3)', opacity: loading ? 0.6 : 1 }}
          >
            ✓ Approve Report
          </button>
        </div>
    ...
    <div className="management-section">
            ...
            <table style={{ minWidth: '1600px' }}>
              ...
              <tbody>
                {pendingReports.map((report, index) => (
                  <PendingReportRow key={index} report={report} index={index} onViewRemarks={handleViewRemarks} />
                ))}
              </tbody>
            </table>
          </div>
      ...
      {showRemarksModal && selectedRemarks && (
        <RemarksDetailModal
          ...
          onApprove={() => handleApproveReport(selectedRemarks.centerCode)}
          onReject={() => handleRejectReport(selectedRemarks.centerCode, selectedRemarks.centerName)}
        />
      )}
    </div>
  );
