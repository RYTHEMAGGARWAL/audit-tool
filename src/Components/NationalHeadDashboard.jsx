...
// This dashboard only shows the four headline counts (unlike the admin
// Dashboard, which also shows email/remarks/closed cards).
const TopStatsGrid = ({ data, activeFilter, onFilter }) => (
...
// Outer role/logout bar — distinct from the inner "Audit Analytics
// Dashboard" title row below.
const TopBar = ({ role, firstname, onLogout }) => (
  <header style={{ background: 'white', padding: '15px 25px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 2px 5px rgba(0,0,0,0.1)', borderBottom: '1px solid #e0e0e0' }}>
    <h1 style={{ margin: 0, fontSize: '20px', fontWeight: '700', color: '#333' }}>
      🏛️ {role} Dashboard - Welcome, {firstname}
    </h1>
    <button onClick={onLogout} style={{ background: '#dc3545', color: 'white', border: 'none', padding: '8px 20px', borderRadius: '6px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>
      Logout
    </button>
  </header>
);

const DashboardHeader = ({ role, lastRefresh, activeFilter, onClearFilter, onRefresh }) => (
  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
    <div>
      <h2 style={{ margin: 0, fontSize: 22, fontWeight: '800', color: '#1a237e' }}>🏛️ {role} — Audit Analytics Dashboard</h2>
...
export default function NationalHeadDashboard() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f2f8', fontFamily: 'Segoe UI, sans-serif' }}>
      ...
      <TopBar role={loggedUser.Role} firstname={loggedUser.firstname} onLogout={() => { localStorage.removeItem('loggedUser'); navigate('/'); }} />
      ...
      <div style={{ padding: 20 }}>
        ...
        <DashboardHeader role={loggedUser.Role} lastRefresh={lastRefresh} activeFilter={activeFilter} onClearFilter={() => { setActiveFilter(null); setReportView('recent'); }} onRefresh={load} />
        <ActiveFilterBanner activeFilter={activeFilter} count={filtered.length} />
        <TopStatsGrid data={data} activeFilter={activeFilter} onFilter={handleFilter} />
