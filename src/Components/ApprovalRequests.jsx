...
// One of the 3 top tab buttons (Center Users / Centers / Reopen Requests)
const SectionToggleButton = ({ label, count, isActive, onClick, activeGradient }) => (
  <button
    onClick={onClick}
    style={{
      padding: '10px 24px', borderRadius: '8px', border: 'none', cursor: 'pointer', fontWeight: 'bold', fontSize: '14px',
      background: isActive ? activeGradient : '#f1f5f9',
...
// Table of before/after values for a pending user's modify request
const UserChangesTable = ({ user }) => {
  ...
  const changedEntries = Object.entries(user.pendingModifyData).filter(([k]) => !skipFields.includes(k));
  if (changedEntries.length === 0) return null;

  const getCurrentValue = (field) => {
    if (field === 'centerCode') return user.centerCode;
    if (field === 'Role' || field === 'role') return user.Role || user.role;
    if (field === 'email') return user.email;
    if (field === 'mobile') return user.mobile;
...
// One pending-user (new user or modify request) card
const PendingUserCard = ({ u, handleApprove, handleReject }) => (
  <div style={{ background: 'white', border: '2px solid #e3f2fd', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '6px', fontSize: '13px', color: '#555' }}>
          ...
          {u.modifyApprovalStatus === 'pending' && u.pendingModifyData && <UserChangesTable user={u} />}
        </div>
      </div>
...
// Table of before/after values for a pending center's edit request
const CenterChangesTable = ({ changedFields }) => (
...
// One pending-center (new center or edit request) card
const PendingCenterCard = ({ c, handleApprove, handleReject }) => {
  const isPending = c.editApprovalStatus === 'pending';
  const hasChanges = isPending && c.changedFields && Object.keys(c.changedFields).length > 0;
  const pendingNoChanges = isPending && !hasChanges;

  return (
    <div style={{ background: 'white', border: `2px solid ${isPending ? '#e3f2fd' : '#e8f5e9'}`, borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
            <span style={{ fontWeight: 'bold', fontSize: '16px', color: isPending ? '#1565c0' : '#2e7d32' }}>
              🏢 {c.centerName} <span style={{ fontSize: '13px', color: '#667eea' }}>({c.centerCode})</span>
            </span>
            <span style={{ padding: '3px 10px', borderRadius: '10px', fontSize: '11px', fontWeight: 'bold',
              background: isPending ? '#e3f2fd' : '#e8f5e9',
              color: isPending ? '#1565c0' : '#2e7d32',
              border: `1px solid ${isPending ? '#2196f3' : '#4caf50'}`
            }}>
              {isPending ? '✏️ Edit Request' : '🆕 New Center'}
            </span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '6px', fontSize: '13px', color: '#555' }}>
            ...
            {hasChanges && <CenterChangesTable changedFields={c.changedFields} />}
            {pendingNoChanges && (
...
// One pending reopen-request card
const ReopenRequestCard = ({ r, loadPending, onUpdate }) => {
  ...
  return (
    <div style={{ background: 'white', border: '2px solid #ff9800', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
        <div style={{ flex: 1 }}>
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '20px' }}>
        <SectionToggleButton
          label="👤 Center Users" count={pendingUsers.length}
          isActive={activeSection === 'users'} onClick={() => setActiveSection('users')}
          activeGradient="linear-gradient(135deg, #1a237e, #3949ab)"
        />
        <SectionToggleButton
          label="🏢 Centers" count={pendingCenters.length}
          isActive={activeSection === 'centers'} onClick={() => setActiveSection('centers')}
          activeGradient="linear-gradient(135deg, #1a237e, #3949ab)"
        />
        <SectionToggleButton
          label="🔓 Reopen Requests" count={reopenRequests.length}
          isActive={activeSection === 'reopen'} onClick={() => setActiveSection('reopen')}
          activeGradient="linear-gradient(135deg, #e65100, #ff9800)"
        />
      </div>
      ...
      {activeSection === 'users' && (
        pendingUsers.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {pendingUsers.map(u => (
              <PendingUserCard key={u._id} u={u} handleApprove={handleApprove} handleReject={handleReject} />
            ))}
          </div>
        )
      )}
        ...
        pendingCenters.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {pendingCenters.map(c => (
              <PendingCenterCard key={c._id} c={c} handleApprove={handleApprove} handleReject={handleReject} />
            ))}
          </div>
        )
      )}
        ...
        reopenRequests.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {reopenRequests.map(r => (
              <ReopenRequestCard key={r._id} r={r} loadPending={loadPending} onUpdate={onUpdate} />
            ))}
          </div>
        )
      )}
